  ...
  if (redApp) {
      ...
      function(req, res) {
        ...
        storage
          ...
          .otherwise(function(err) {
            if (err) {
              log.warn(
                log._('api.library.error-load-entry', {
                  path: path,
              ...
              if (err.code === 'forbidden') {
                log.audit(
                  { event: 'library.get', type: type, error: 'forbidden' },
                  req
                )
                res.status(403).end()
                return
            ...
            log.audit(
              { event: 'library.get', type: type, error: 'not_found' },
              req
            )
            res.status(404).end()
          })
    ...
    redApp.post(
      ...
      function(req, res) {
        ...
        storage
          .saveLibraryEntry(type, path, meta, text)
          .then(function() {
          ...
          .otherwise(function(err) {
            ...
            if (err.code === 'forbidden') {
              log.audit(
                { event: 'library.set', type: type, error: 'forbidden' },
                req
              )
              res.status(403).end()
              return
...
module.exports = {
  init: function(app, runtime) {
    ...
    runtime.events.removeListener(
      'node-module-uninstalled',
      removeNodeExamplesDir
    )
    ...
    if (req.params[0].indexOf('_examples_/') === 0) {
      ...
      if (m) {
        ...
        if (exampleRoots[module]) {
          ...
          try {
            ...
            log.audit(
              { event: 'library.get', type: 'flow', path: req.params[0] },
              req
            )
      ...
      storage
        ...
        .otherwise(function(err) {
          if (err) {
            log.warn(
              log._('api.library.error-load-flow', {
                path: req.params[0],
                message: err.toString()
              })
            )
  ...
  post: function(req, res) {
    ...
    storage
      .saveFlow(req.params[0], flow)
      .then(function() {
        log.audit(
          { event: 'library.set', type: 'flow', path: req.params[0] },
          req
        )
        res.status(204).end()
      })
      .otherwise(function(err) {
        log.warn(
          log._('api.library.error-save-flow', {
            path: req.params[0],
            message: err.toString()
          })
        )
        ...
        res
          .status(500)
          .send({ error: 'unexpected_error', message: err.toString() })
      })
  }
}
