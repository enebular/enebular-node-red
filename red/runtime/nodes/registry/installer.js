...
function installModule(module) {
  ...
  return when.promise(function(resolve, reject) {
    ...
    var child = child_process.execFile(
      npmCommand,
      ['install', '--production', installName],
      {
        cwd: installDir
      },
      function(err, stdin, stdout) {
        if (err) {
          ...
          if (lookFor404.test(stdout)) {
            log.warn(
              log._('server.install.install-failed-not-found', { name: module })
            )
            ...
            log.warn(
              log._('server.install.install-failed-long', { name: module })
            )
            log.warn('------------------------------------------')
...
function uninstallModule(module) {
  return when.promise(function(resolve, reject) {
    ...
    try {
      ...
      return reject(
        new Error(log._('server.install.uninstall-failed', { name: module }))
      )
    }
    ...
    var child = child_process.execFile(
      ...
      function(err, stdin, stdout) {
        if (err) {
          log.warn(
            log._('server.install.uninstall-failed-long', { name: module })
          )
          log.warn('------------------------------------------')
          log.warn(err.toString())
          log.warn('------------------------------------------')
          reject(
            new Error(
              log._('server.install.uninstall-failed', { name: module })
            )
          )
        } else {
